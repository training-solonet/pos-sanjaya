...
<script>
  ...
  function changeChartPeriod(period) {
    ...
    buttons.forEach(btn => {
      if (btn.textContent.trim() === (period === "7days" ? "7 Hari" : "30 Hari")) {
        btn.classList.remove("bg-gray-100", "text-gray-600");
        btn.classList.add("active", "bg-green-100", "text-green-600");
      }
    });
  ...
  function updateSalesStats(period) {
    ...
    if (avgDailyElement) {
      avgDailyElement.textContent = "Rp " + Math.round(avgDaily).toLocaleString("id-ID");
    }
    if (totalTransactionsElement) {
      totalTransactionsElement.textContent = totalTransactions.toString();
    }
  }
  ...
  // Reinitialize charts when the page is fully loaded
  window.addEventListener('load', function() {
    setTimeout(() => {
      const salesChartCanvas = document.getElementById("salesChart");
      if (salesChartCanvas) {
        // Recreate chart to ensure proper sizing
        if (currentChart) {
          currentChart.destroy();
        }
        createSalesChart(currentPeriod);
      }
    }, 100);
  });
</script>
