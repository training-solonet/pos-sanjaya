...
<!-- Custom Confirmation Modal (Mengganti alert JavaScript) -->
<div id="customConfirmModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md animate-fade-in">
            <div class="p-6">
                <div class="flex items-center space-x-3 mb-4">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900" id="confirmModalTitle">Konfirmasi Hapus</h3>
                </div>
                <p class="text-sm text-gray-600 mb-6" id="confirmModalMessage">
...
<script>
    ...
    async function editTransaction(id) {
      try {
        ...
        showErrorNotification('Gagal memuat data transaksi');
      }
    }

    // Delete transaction dengan custom modal
    ...
    async function confirmDelete() {
      ...
      try {
        const response = await fetch(`/management/jurnal/${transactionToDelete}`, {
          method: 'DELETE',
          headers: {
        ...
        if (data.success) {
          ...
          showErrorNotification('Gagal menghapus transaksi: ' + data.message);
        }
      } catch (error) {
        console.error('Error deleting transaction:', error);
        showErrorNotification('Gagal menghapus transaksi: ' + error.message);
      } finally {
        closeConfirmModal();
      }
    }
    ...
    document.getElementById('transactionForm').addEventListener('submit', async function(e) {
      ...
      if (formData.nominal <= 0) {
        showErrorNotification('Nominal harus lebih dari 0!');
        return;
      }
      ...
      try {
        ...
        showErrorNotification('Gagal menyimpan transaksi: ' + error.message);
        submitButton.textContent = originalText;
        submitButton.disabled = false;
      }
    });
    ...
    function showExportLoading() {
      ...
      if (!loadingDiv) {
        ...
        loadingDiv.className = 'fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center animate-fade-in';
    ...
    // Fungsi notifikasi yang sama seperti di halaman kasir
    ...
    function showNotification(message, type) {
      ...
      notification.className = `custom-notification fixed top-4 right-4 ${bgColor} text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center space-x-2 animate-fade-in`;
      ...
      // Hapus notifikasi setelah 3 detik dengan animasi fade out
      setTimeout(() => {
