                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Expired Date
                                    </th>
                                    ...
                                    <tr class="product-row"
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center space-x-3">
                                                ...
                                                <div>
                                                    ...
                                                    <span
                                                        class="text-xs text-gray-500">{{ $item->bahan_baku->nama ?? 'Umum' }}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td
                                            class="px-6 py-4 text-sm {{ $item->stok == 0 ? 'text-danger font-medium' : ($item->stok <= $item->min_stok ? 'text-orange-600 font-medium' : 'text-gray-900') }}">
                                            {{ $item->stok }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4">
                                            @php
                                                ...
                                                $daysUntilExpired = $today->diffInDays($expiredDate, false);
                                                $isExpired = $daysUntilExpired < 0;
                                                $isExpiredToday = $daysUntilExpired === 0;
                                                $isExpiredSoon = $daysUntilExpired > 0 && $daysUntilExpired <= 3;
                                            @endphp
                                            <div class="text-sm">
                                                <span
                                                    class="{{ $isExpired ? 'text-red-600 font-medium' : ($isExpiredToday ? 'text-orange-600 font-medium' : ($isExpiredSoon ? 'text-orange-600 font-medium' : 'text-gray-900')) }}">
                                                    {{ $expiredDate->format('d M Y') }}
                                                </span>
                                                <div
                                                    class="text-xs {{ $isExpired ? 'text-red-600' : ($isExpiredToday ? 'text-orange-600' : ($isExpiredSoon ? 'text-orange-600' : 'text-gray-500')) }}">
                                                    @if ($isExpired)
                                                        Sudah expired
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-lg w-full max-w-md">
                ...
                <form id="editProductForm" class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Bahan Baku</label>
                        <select name="id_bahan_baku" id="edit_id_bahan_baku" required
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500">
                            <option value="">Pilih Bahan Baku</option>
                            @foreach ($bahan_baku as $bahan)
                                <option value="{{ $bahan->id }}">{{ $bahan->nama }}</option>
